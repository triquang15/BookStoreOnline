...
<body>
			...
			<table>
				...
				<tr>
					...
					<td><select name="paymentMethod">
							<option value="Cash On Delirery"
								<c:if test="${order.paymentMethod eq 'Cart on Delivery'}">selected='selected'</c:if>>Cash
								On Delivery</option>
							<option value="paypal"
								<c:if test="${order.paymentMethod eq 'paypal'}">selected='selected'</c:if>>PayPal
								or Credit Cart</option>
					</select></td>
				</tr>
				<tr>
					<td><b>Ship To: </b></td>
					...
					<tr>
						...
						<td><input type="hidden" name="price"
							value="${orderDetail.book.price}" /> $<fmt:formatNumber
								value="${orderDetail.book.price}" /></td>
						...
						<td><a
							href="remove_book_form_order?id=${orderDetail.book.bookId}">Remove</a></td>
					</tr>
				</c:forEach>
				<tr>
	...
	<script>
		function showAddBookPopup(){
			...
			var top = (screen.hight - hight) / 2;
			window.open('add_book_form', '_blank', 
					'width=' + width + ', height=' + hight + ', top=' + top + ', left=' + left);
			}
		...
		$(document).ready(function() {
			$("#orderForm").validate({
				...
				messages : {
					...
					address1 : "Please enter shipping address line 1",
					address2 : "Please enter shipping address line 2",
					phone : "Please enter phone",
					city : "Please enter city",
					state : "Please enter pstatehone",
